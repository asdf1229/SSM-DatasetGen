...
Before each run, the pipeline removes outputs generated by previous runs:
`configs/ofat_tasks/`, `datasets/real/`, `datasets/synthetic/`,
`datasets/queries/`, and generated manifest/report CSV files under
`datasets/manifests/`. Raw inputs under `datasets/raw/` are preserved.
...
standard `.txt` files under `datasets/real/`.
